...
from pgadmin.browser.register_browser_preferences import register_browser_preferences
...
SMTP_SOCKET_ERROR = gettext("SMTP Socket error: {error}\n {pass_error}").format(
...
class BrowserPluginModule(PgAdminModule, metaclass=ABCMeta):
    ...
    def __init__(self, import_name, **kwargs):
        ...
        super().__init__("NODE-%s" % self.node_type, import_name, **kwargs)
    ...
    def csssnippets(self):
        ...
        """
        snippets = [
            render_template("browser/css/node.css", node_type=self.node_type, _=gettext)
        ]
...
def _get_supported_browser():
    ...
    # comparing EdgeHTML engine version
        ...
        # EdgeHTML version
        engine_to_actual_browser_version = {16: 41, 17: 42, 18: 44}
...
def gravatar(username):
    ...
    """
    g = Gravatar(username)
    return g.get_image(size=100, rating="g", default="retro")
...
def index():
    ...
    if (
        config.SERVER_MODE
        and not config.MASTER_PASSWORD_REQUIRED
        and "pass_enc_key" in session
    ):
        session["allow_save_password"] = False

    response = Response(
        render_template(
            MODULE_NAME + "/index.html", username=current_user.username, _=gettext
        )
    )
    ...
    domain = dict()
    if config.COOKIE_DEFAULT_DOMAIN and config.COOKIE_DEFAULT_DOMAIN != "localhost":
...
def validate_shared_storage_config(data, shared_storage_keys):
    ...
    if shared_storage_keys.issubset(data.keys()):
        if (
            ...
            and isinstance(data["restricted_access"], bool)
        ):
            return True
    return False
...
def get_shared_storage_list():
    ...
    if config.SERVER_MODE:
        ...
        shared_storage_config = [
            sdir
            for sdir in config.SHARED_STORAGE
            if validate_shared_storage_config(sdir, shared_storage_keys)
        ]

        config.SHARED_STORAGE = shared_storage_config
        shared_storage_list = [sdir["name"] for sdir in shared_storage_config]
        restricted_shared_storage_list = [
            sdir["name"] for sdir in shared_storage_config if sdir["restricted_access"]
        ]
...
def utils():
    ...
    highlight_selection_matches_pref = prefs.preference("highlight_selection_matches")
    ...
    shared_storage_list, restricted_shared_storage_list = get_shared_storage_list()
...
def get_config():
    ...
    return make_json_response(
        data={
            "csrf_token_header": current_app.config.get("WTF_CSRF_HEADERS")[0],
            "csrf_token":  generate_csrf(),
            "user_inactivity_timeout": current_app.config["USER_INACTIVITY_TIMEOUT"],
            "override_user_inactivity_timeout": current_app.config.get(
                "OVERRIDE_USER_INACTIVITY_TIMEOUT"
            ),
            "enable_psql": config.ENABLE_PSQL,
            "shared_storage": shared_storage_list,
            "platform": sys.platform,
            "qt_default_placeholder": QT_DEFAULT_PLACEHOLDER,
            "vw_edt_default_placeholder": VW_EDT_DEFAULT_PLACEHOLDER,
            "enable_binary_path_browsing": current_app.config.get(
                "ENABLE_BINARY_PATH_BROWSING"
            ),
            "fixed_binary_paths": current_app.config.get("FIXED_BINARY_PATHS"),
            "pgadmin_server_locale": default_locale,
            "heartbeat_timeout": config.SERVER_HEARTBEAT_TIMEOUT,
            "password_length_min": config.PASSWORD_LENGTH_MIN,
            "enable_server_passexec_cmd": config.ENABLE_SERVER_PASS_EXEC_CMD,
            ...
            'wrapCode': editor_wrap_code,
            'useSpaces': editor_use_spaces,
            'insertPairBrackets':insert_pair_brackets,
            'braceMatching':brace_matching,
            'highlightSelectionMatches': highlight_selection_matches,
            'is_indent_with_tabs': editor_indent_with_tabs,
            'app_name': config.APP_NAME,
            'app_version_int': config.APP_VERSION_INT,
            'pg_libpq_version': pg_libpq_version,
            'support_ssh_tunnel': config.SUPPORT_SSH_TUNNEL,
...
def browser_css():
    ...
    for submodule in blueprint.submodules:
        snippets.extend(submodule.csssnippets)
    return make_response(
        render_template("browser/css/browser.css", snippets=snippets, _=gettext),
        200,
...
@blueprint.route(
    "/check_corrupted_db_file", endpoint="check_corrupted_db_file", methods=["GET"]
)
def check_corrupted_db_file():
    ...
    file_location = (
        ...
        if "CORRUPTED_DB_BACKUP_FILE" in os.environ
        else ""
    )
...
@blueprint.route("/master_password", endpoint="check_master_password", methods=["GET"])
...
@blueprint.route("/master_password", endpoint="set_master_password", methods=["POST"])
def set_master_password():
            ...
            try:
                ...
                if not master_key:
                    ...
                    keyring.delete_password(
                        KEY_RING_SERVICE_NAME, "entry_to_check_keychain_access"
                    )
                    # migrate existing server passwords
                    from pgadmin.browser.server_groups.servers.utils import (
                        migrate_saved_passwords,
                    )
                    ...
                    if migrated_save_passwords:
                        ...
                        return form_master_password_response(
                            existing=True, present=True, keyring_name=keyring_name
                        )
                    else:
                        if not error:
                            # Update keyring
                ...
                error = (
                    "Failed to get/set encryption key using OS password "
    ...
    if (
        ...
        or OAUTH2 in config.AUTHENTICATION_SOURCES
        ...
        if (
            ...
            and data.get("submit_password", False)
            and not validate_master_password(data.get("password"))
        ):
        ...
        if data != "" and data.get("password", "") != "":
            ...
            if current_user.masterpass_check is None:
                ...
                reencrpyt_server_passwords(
                    current_user.id, current_user.password, data.get("password")
                )
        ...
        elif not get_crypt_key()[0] and current_user.masterpass_check is not None:
            return form_master_password_response(
                existing=True, present=False, keyring_name=keyring_name
            )
    ...
    if config.SERVER_MODE and current_user.masterpass_check is None:
        ...
        reencrpyt_server_passwords(current_user.id, current_user.password, crypt_key)
...
if hasattr(config, "SECURITY_CHANGEABLE") and config.SECURITY_CHANGEABLE:
    ...
    @blueprint.route(
        "/change_password", endpoint="change_password", methods=["GET", "POST"]
    )
    @pgCSRFProtect.exempt
    @pga_login_required
    def change_password():
        ...
        return make_json_response(
            success=1, info=gettext("pgAdmin user password changed successfully")
        )
...
if hasattr(config, "SECURITY_RECOVERABLE") and config.SECURITY_RECOVERABLE:
    ...
    def send_reset_password_instructions(user):
        ...
        reset_link = url_for("browser.reset_password", token=token, _external=True)
    ...
    @blueprint.route(
        "/reset_password", endpoint="forgot_password", methods=["GET", "POST"]
    )
    @pgCSRFProtect.exempt
    ...
    def forgot_password():
        ...
        if form.validate_on_submit():
            ...
            user = User.query.filter_by(
                email=form.data["email"], auth_source=INTERNAL
            ).first()
    ...
    @blueprint.route(
        "/reset_password" + slash_url_suffix("/browser/reset_password", "<token>"),
        methods=["GET", "POST"],
    ...
    def reset_password(token):
        ...
        if form.validate_on_submit():
            ...
            if not has_error:
                ...
                return redirect(
                    current_app.config["SECURITY_POST_RESET_VIEW"]
